...
(deftest next-cells-test
  (testing "A world with no cells returns the empty set."
    (is (= #{} (next-cells #{} []))))

  (testing "A world with one cell returns the empty set."
    (let [cells #{[1,1]}
...
(deftest live-test
  (let [alive true
    ...
    (testing "A cell with less than two neighbors dies of underpopulation."
